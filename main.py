...
# Хранилище активных звонков и WebSocket соединений для звонков
active_calls: Dict[str, Dict] = {}
call_websockets: Dict[str, WebSocket] = {}  # username -> websocket для звонков
...
async def websocket_call_endpoint(websocket: WebSocket, username: str):
    """WebSocket для сигнализации звонков (отдельный от основного)"""
    await websocket.accept()
    call_websockets[username] = websocket
    print(f"Call WebSocket connected: {username}")
    ...
    try:
        while True:
            ...
            if data["type"] == "call_offer":
                # Исходящий звонок
                target = data["target"]
                if target in call_websockets:
                    # Пересылаем предложение целевому пользователю
                    await call_websockets[target].send_json({
                        "type": "incoming_call",
                        "caller": username,
                        "offer": data["offer"]
                    })
                ...
                if target in call_websockets:
                    ...
                    if data["call_id"] in active_calls:
                        active_calls[data["call_id"]]["status"] = "connected"
            
            elif data["type"] == "ice_candidate":
                # ICE кандидат для установки соединения
                target = data["target"]
                if target in call_websockets:
                    await call_websockets[target].send_json({
                        "type": "ice_candidate",
        ...
        print(f"Call WebSocket disconnected: {username}")
        if username in call_websockets:
            del call_websockets[username]
        # Завершаем все активные звонки этого пользователя
        calls_to_remove = []
        for call_id, call in active_calls.items():
            if call["caller"] == username or call["callee"] == username:
                other = call["callee"] if call["caller"] == username else call["caller"]
                if other in call_websockets:
                    await call_websockets[other].send_json({
                        "type": "call_ended",
                        "call_id": call_id
                    })
